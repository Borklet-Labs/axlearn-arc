...
# Check for commit / branch name
if [ -n "$CUSTOM_GIT_COMMIT" ]; then
    TARGET_REF="$CUSTOM_GIT_COMMIT"
elif [ -n "$CUSTOM_GIT_BRANCH" ]; then
    TARGET_REF="$CUSTOM_GIT_BRANCH"
else
    TARGET_REF="main"
fi
...
gsutil -m cp /var/arc/result.csv ${GCS_PREFIX}/results/training-test-${ACCELERATOR}-${GITHUB_HASH}-${JAX_VER}-${GH_RUN_ID}-${TIMESTAMP}.csv