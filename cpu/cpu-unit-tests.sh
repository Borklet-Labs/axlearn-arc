...
# Iterate over the tests in axlearn
for test in "${gcp_array[@]}"; do
    # Change the / in the filename to - for readability
    export output_name=$(sed 's/\//\-/g' <<< "$test")
    echo "[CPU] Testing $test..."
    # Get the XML output and the Allure results for easier reading
    pytest -v --junit-xml=/home/runner/_work/xml_results/$output_name.xml --alluredir /home/runner/_work/allure_results -n 8 $test
done
